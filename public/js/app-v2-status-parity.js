(() => {
  ...
  app.statusLabels = function mondayStatusLabels(column) {
    ...
    return originalStatusLabels(column).map((entry, index) => ({ ...entry, id: String(entry.id ?? index), position: index }));
  };
  ...
  app.openDynamicStatusMenu = function openMondayStatusMenu(anchor, itemId, columnId) {
    ...
    [...labels, { id: '__empty__', label: '', color: '#c4c4c4' }].forEach(option => {
  ...
  app.statusLabelEditorRowHtml = function statusLabelEditorRowHtml(label, index) {
    ...
    return `<div class="status-label-editor-row" data-status-label-id="${this.escapeAttr(id)}">
      <span class="status-drag-handle" title="Orden de etiqueta">⋮⋮</span>
  ...
  app.openStatusLabelsEditor = function openStatusLabelsEditor(columnId) {
    ...
    this.openModal(`<form id="status-labels-form" class="modal-card status-labels-modal">
      ...
      <p class="status-label-note">El estado gris vacío se mantiene como estado predeterminado, igual que en Monday.</p>
    ...
    const bindRemove = row => row.querySelector('[data-status-label-remove]')?.addEventListener('click', () => row.remove());
    list.querySelectorAll('.status-label-editor-row').forEach(bindRemove);
    ...
    document.getElementById('status-add-label')?.addEventListener('click', () => {
      ...
      list.insertAdjacentHTML('beforeend', this.statusLabelEditorRowHtml({ id: `local_${Date.now()}_${index}`, label: '', color: STATUS_COLORS[index % STATUS_COLORS.length] }, index));
